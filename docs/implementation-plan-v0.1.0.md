...
- [ ] Track playback for PREPARE spoken/unspoken accounting (conservative sentence-level):
...
#### M3.7 — prepare.py (PREPARE handler — crosses voice + inference)
...
| OQ5 | ~~PREPARE token accounting precision.~~ Resolved: conservative sentence-level accounting (undercount by ≤1 sentence). Phase 2 path: `on_word` callback for exact word-level tracking.                                                                  | Resolved in M3.6.                                                                                                      |
