...
└────────────────────────────┬─────────────────────────────────────┘
         ...
         ▼                                                       ▼
┌────────────────────────────────┐          ┌───────────────────────────┐
│  Process 2: Talker (Python)     │          │  Process 4+: Reasoner(s)  │
...
- [ ] Define `GatewayState` struct:
  ...
  - `silence_timer: Option<JoinHandle>` (one at a time per spec §8)
...
- [ ] Implement `TalkerServiceServer`: stubs only at this stage — accept calls, return empty responses.
...
- [ ] On `vad_speech_start` [P2]: dispatch `Prepare` gRPC to Talker; cancel silence timers.
- [ ] On `final_transcript` [P1]: assemble `TalkerContext`; dispatch `ProcessPrompt` gRPC.
- [ ] On `TalkerOutput.ResponseComplete`: append to history; dispatch `PrefillCache` to Talker; start silence timer.
...
- [ ] On silence timer fire [P4]: assemble minimal context package; dispatch to Talker.
...
~50-100 lines. Processes one complete sentence. Pure function (stateless, deterministic):
- [ ] `process(sentence: str) -> SoulContainerResult` where result contains:
...
│   │   ├── voice/
│   │   │   ├── __init__.py
│   │   │   ├── listener.py         # RealtimeSTT wrapper: VAD + STT callbacks → gRPC events
