...
**Progress evidence:** Source inspection on `gateway-refactor`, based on commit `633ec49` plus the accepted REF-024 documentation changes. Runtime tests were not rerun for this progress update.
...
- [x] In-memory `History`, turn state, context assembly, persona loading/watching and silence timers exist. Only SOUL.md and IDENTITY.md are watched. Sources: [core](../gateway/src/core/), [startup](../gateway/src/main.rs).
...
| --- | --- | --- | --- |
| R0 | Structural extraction and documentation alignment | Existing baseline | Pending |
...
- [ ] Extract construction/configuration/startup wiring to `gateway/src/app.rs`; keep `main.rs` as entry point (REF-024).
- [ ] Extract the existing event loop into `gateway/src/core/pipeline/run.rs`, retaining P0's separate control branch and existing handler/action boundaries.
- [ ] Keep concrete provider setup in assembly or provider-local factories; pipeline code consumes capability handles. Core session/policy/task/workspace modules remain ordinary internal APIs.
- [ ] Keep provider background work under the existing lifecycle owner; preserve Supervisor process/sandbox ownership.
- [ ] Keep new responsibilities as files in the existing structure; update `lib.rs` exports and `gateway/structure.txt` to match the actual extraction.
- [x] Reconcile stale Gateway spec claims about process spawning, client/server direction, interruption protocol, session identity, transport, memory behavior, and actual versus planned audio egress. Documentation updated on 2026-09-17; code extraction and runtime acceptance remain pending.
- [ ] Identify blocking/long awaits on the P0 path and record the required scheduling/cancellation correction separately from mechanical file moves; preserve the responsiveness requirement.
