...
- The Gateway is the only conversational component that touches persistent application/workspace files (RAG SQLite store, SOUL.md, IDENTITY.md, workspace tools). The Supervisor-owned Sandbox Provider may manage only its isolated scratch/container filesystem.
...
- [ ] **M0.3** Create `proto/buf.gen.yaml` for all three target languages:
  ...
  Rust stubs for Gateway and Supervisor are generated by `tonic-build` in
  `build.rs` using `protoc-bin-vendored`, not by buf or a system `protoc`.
...
**Goal:** Tool execution through a Supervisor-owned Sandbox Provider, results returned to Gateway's normal P3 tool-result path.

- [ ] Init `tools/` as a Node.js/TypeScript project (`package.json`, `tsconfig.json`).
- [ ] Implement Phase 1 tools (workspace paths remain enforced by Gateway; code execution uses an opaque Supervisor sandbox handle):
...
- [ ] Expose capability tools through the Gateway registry; `sandbox_exec` calls Supervisor's acquire/execute/release control-plane API.
- [ ] **[DECISION]** Gateway owns tool semantics and P3 correlation. Supervisor owns Sandbox Provider construction, backend policy, handles, resource limits, prewarming, and teardown (REF-015).
